...
% Slide 7: Why CPR? Nonlinear Cointegration and the I(2) Trap
\begin{frame}{Why CPR? Nonlinear Cointegration and the $I(2)$ Trap}
  \begin{itemize}
    \item \textbf{The $I(2)$ Interaction Trap:} 
      If $\tilde{\tau}_t \sim I(1)$ and $\tilde{\omega}_t \sim I(1)$, their product $(\tilde{\tau}_t \cdot \tilde{\omega}_t)$ is integrated of order two ($I(2)$). Standard linear cointegration critical values are asymptotically invalid.
    \item \textbf{Stochastic Trend Cancellation:} 
      CPR theory proves that the $I(2)$ component of the interaction cancels out with the stochastic components of $y_t$, yielding a stationary ($I(0)$) error term.
    \item \textbf{Asymptotic Bias Correction:} 
      Naive OLS is superconsistent but suffers from endogeneity and serial correlation. 
      \begin{itemize}
        \item \textbf{FM-OLS:} Corrects covariance terms non-parametrically.
        \item \textbf{IM-OLS:} Adds lead/lag adjustments directly to absorb short-run dynamics, avoiding unstable covariance estimation of product terms.
      \end{itemize}
    \item \textbf{VIF Stabilization:} 
      Residual centering regresses the interaction on main effects to isolate the orthogonalized residual. This resolves collinearity (VIF = 1.0) without shifting the underlying cointegrating space.
  \end{itemize}
\end{frame}

% Slide 8: Results: Specification A (Scale-Conditioning)
...
\begin{frame}{Unpacking Specification B: Golden Age Case Study (1945--1973)}
  \begin{block}{Estimated Cointegrating Equation (FM-OLS)}
    $$y_t = 14.8617 + 0.2378 \tilde{k}^{scale}_t + 0.3121 \tilde{\tau}_t - 5.1605 \tilde{\omega}_t - 1.0294 (\tilde{\tau}_t \cdot \tilde{\omega}_t)_{orth}$$
  \end{block}
  \begin{itemize}
