...
% Tighten vertical spacing globally for Beamer frames
\setlength{\parskip}{0pt}
\setbeamertemplate{itemize/enumerate body begin}{\vspace{-0.2em}}
\setbeamertemplate{itemize/enumerate subbody begin}{\vspace{-0.2em}}
...
\begin{frame}{The Extensive Margin ($K^{NRC}_t$)}
  ...
  \begin{itemize}
    ...
    \item \textbf{Asset Role:} In this framework, structures act as the direct physical scale parameter for capacity ($K_t^{scale} \equiv K_t^{NRC}$).
...
% Slide 5: Technical Choice under Unbalanced Growth
\begin{frame}{Technical Choice under Unbalanced Growth}
  \begin{block}{The Technology Frontier}\small
    Productive capacity $Y^p_t$ is generated on a concave technology frontier---not a neoclassical production function determining factor shares, but a physical capacity schedule mapping capital investment into maximum achievable output under distribution conflict:\vspace{-0.3em}
    $$Y^p_t = F(K^{NRC}_t, K^{ME}_t; \omega_t) = A_t (K^{NRC}_t)^{\theta} (K^{ME}_t)^{\psi + \lambda \omega_t}$$
  \end{block}\vspace{-0.3em}
  \begin{itemize}\itemsep0.3em
    ...
    \item \textbf{Unbalanced Gestation:} The cost-minimization FOC yields:\vspace{-0.3em}
      $$\theta^{\tau}_t \equiv \frac{\partial \ln Y^p_t}{\partial \ln \tau_t} = \psi + \lambda \omega_t$$\vspace{-0.6em}
    \item \textbf{Frontier Concavity:} Diminishing returns to mechanization imply:\vspace{-0.3em}
      $$\lambda \equiv \frac{\partial^2 \ln Y^p_t}{\partial \ln \tau_t \partial \omega_t} < 0$$
  \end{itemize}
\end{frame}

% Slide 6: Econometric Specifications
...
\begin{frame}{Capacity Utilization \& Elasticity Reconstruction (1945--1973)}
  ...
  \begin{itemize}\itemsep0.2em
    \item \textbf{Sectoral Division:} Elasticities estimated on NFC data; utilization ($\mu_t$) reconstructed relative to \textbf{Total Real GDP}.
    \item \textbf{Historical Path Comparison:}
      \begin{itemize}\itemsep0.1em
        \item \textbf{Spec B:} Captures utilization fluctuations ($\sigma_{\mu} = 12.98\%$), anchored at 1973 peak ($\mu_{1973}=1.0$).
        \item \textbf{Shaikh-Style:} Higher volatility ($\sigma_{\mu} = 15.06\%$) due to the constant scale assumption.
        \item \textbf{HP Filter:} Dampens real capacity squeezes ($\sigma_{\mu} = 3.79\%$) by absorbing demand booms into trend.
      \end{itemize}
    \item \textbf{Cross-Method Correlation:} Spec B correlates with Shaikh ($\rho = 0.70$) and HP cycle ($\rho = 0.40$).
    \item \textbf{Mechanization Bias \& Cushioning:}
      \begin{itemize}\itemsep0.1em
        \item Structures elasticity is negative ($\theta^{NRC}_t \approx -0.07$): empty plants without machinery yield negative capacity payoff.
        \item Aggregate elasticity ($\theta^{total}_t$) is positive and dynamic (\textbf{0.84}--\textbf{2.30}).
        \item In the 1960 slowdown, Spec B ($\mu_{1960} = 1.19$) cushions the Shaikh collapse ($\mu_{1960} = 0.86$) via slow capacity envelope expansion.
      \end{itemize}
  \end{itemize}
\end{frame}
