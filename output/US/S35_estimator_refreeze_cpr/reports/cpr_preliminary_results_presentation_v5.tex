...
% Slide 4: The Intensive Margin
\begin{frame}{The Intensive Margin ($\tau \equiv K^{ME} / K^{NRC}$)}
  \begin{block}{Ontological Definition}
    The capital composition ratio ($\tau$) represents the choice of technique—the machinery and equipment ($K^{ME}$) installed per unit of physical structure ($K^{NRC}$).
  \end{block}
  \begin{itemize}
...
\begin{frame}{The Cost-Minimization FOC under Unbalanced Growth}
  \begin{block}{The Transformation Frontier}
    Productive capacity $Y^p$ is generated on a concave transformation frontier:
    $$Y^p_t = F(K^{NRC}_t, K^{ME}_t; \omega_t) = A_t (K^{NRC}_t)^{\theta} (K^{ME}_t)^{\psi + \lambda \omega_t}$$
  \end{block}
  \begin{itemize}
    \item \textbf{Induced Innovation:} Wage pressure ($\omega$) forces capitalists to increase machinery density ($\tau \equiv K^{ME} / K^{NRC}$) to save on labor costs.
...
\begin{frame}{The Estimator Design Matrix}
  ...
  \end{block}
  \begin{block}{Specification B: Composition-Mediated (A03 Growth-Law Basis)}
    $$\ln Y_t = \alpha + \theta \tilde{k}^{scale}_t + \psi \tilde{\tau}_t + \phi \tilde{\omega}_t + \lambda (\tilde{\tau}_t \cdot \tilde{\omega}_t)_{orth} + \epsilon_t$$
  ...
  \begin{itemize}
    \item \textbf{Residual Centering (Orthogonalization):}
      ...
      $$(\tilde{\tau}_t \cdot \tilde{\omega}_t)_{orth} = (\tilde{\tau}_t \cdot \tilde{\omega}_t) - \hat{\delta}_1 \tilde{k}^{scale}_t - \hat{\delta}_2 \tilde{\tau}_t - \hat{\delta}_3 \tilde{\omega}_t$$
  \end{itemize}
\end{frame}
...
\begin{frame}{Results: Specification A (Single-Capital Baseline)}
  \begin{table}[htbp]
    ...
    \begin{tabular}{lcccc}
      ...
      \midrule
      \textbf{Golden Age} & $k_{Kcap, centered}$ & 0.9984 & 18.2751 & 0.0000 \\
      (1945--1973) & $\omega_{NFC, centered}$ & -0.5824 & -0.4052 & 0.6881 \\
      \midrule
      \textbf{Post-War Fordist} & $k_{Kcap, centered}$ & 0.9821 & 24.1122 & 0.0000 \\
      (1940--1973) & $\omega_{NFC, centered}$ & -0.7104 & -0.5211 & 0.6045 \\
      \midrule
      \textbf{Post-Fordist} & $k_{Kcap, centered}$ & 1.0543 & 35.8821 & 0.0000 \\
      (1974--2024) & $\omega_{NFC, centered}$ & -4.8824 & -2.8912 & 0.0051 \\
      \bottomrule
    \end{tabular}
  \end{table}
  \begin{itemize}
