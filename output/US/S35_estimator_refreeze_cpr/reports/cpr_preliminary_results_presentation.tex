...
\begin{frame}{The Intensive Margin ($K^{ME}/L$) \& Technique Choice}
  \begin{block}{Ontological Definition}
    Machinery and equipment ($K^{ME}$) paired with labor ($L$) defines the tactical margin of accumulation. Capitalists optimize the mechanization rate $q = K^{ME}/L$ along the frontier.
  \end{block}
  \begin{itemize}
    \item \textbf{Tactical Cost-Minimization Problem:}
      $$\min_{q} \ C = a - \pi \cdot q \quad \text{s.t.} \quad a = \lambda_0^{[s]} + \lambda_1^{[s]} q + \lambda_2^{[s]} q^2$$
      where $a = Y/L$ (labor productivity growth), and $\pi = 1-\omega$ (profit share).
    ...
    \item \textbf{Induced Innovation:} Since the frontier is concave ($\lambda_2^{[s]} < 0$):
      $$\frac{\partial q^*}{\partial \omega} = -\frac{1}{2\lambda_2^{[s]}} > 0$$
      Rising wage share ($\omega \uparrow \implies \pi \downarrow$) forces capitalists to accelerate mechanization growth ($q^* \uparrow$).
  \end{itemize}
\end{frame}
...
\begin{frame}{Reconciliation: Diminishing Capacity Payoffs}
  \begin{itemize}
    \item Differentiating the capacity identity $Y^p_t = \frac{a^N}{q^*} K^{ME}_t + \Gamma(K^{NRC}_t)$ yields the aggregate transformation elasticity $\theta_t$:
    ...
    \item Differentiating with respect to the optimal mechanization rate $q^*$:
    ...
    \item \textbf{Verdict:} The interaction coefficient between composition ($\tau = k^{ME} - k^{NR}$) and wage share ($\omega$) must be \textbf{negative}.
  \end{itemize}
\end{frame}
...
\begin{frame}{Empirical Specifications}
  \begin{itemize}
    ...
    \item \textbf{The Econometric Resolution:}
      \begin{itemize}
        \item Regressors are mixed-order integrated: the interaction term behaves as $I(2)$ / $I(2)$ risk.
        \item We use \textbf{IM-OLS} and \textbf{FM-OLS} to obtain superconsistent, asymptotically valid inferences.
        \item \textbf{Residual Centering:} Regressing $(\tilde{\tau}_t \tilde{\omega}_t)$ on $\tilde{k}^{scale}_t, \tilde{\tau}_t, \tilde{\omega}_t$ extracts the orthogonalized interaction, reducing VIFs to exactly 1.0 without changing OLS residuals.
      \end{itemize}
  \end{itemize}
\end{frame}

% Slide 7: Curated Results: Specification A (Scale-Conditioning)
\begin{frame}{Curated Results: Specification A (Scale-Conditioning)}
  \begin{table}[htbp]
    \centering
  ...
  \end{table}
  \begin{itemize}
    \item Residuals pass Engle-Granger unit root tests across windows (EG p-value $\le$ 0.10).
    \item Scale coefficient is positive and robustly close to 1.0.
  \end{itemize}
\end{frame}

% Slide 8: Curated Results: Specification B (Composition-Mediated)
\begin{frame}{Curated Results: Specification B (Composition-Mediated)}
  \begin{table}[htbp]
    \centering
  ...
  \end{table}
  \begin{itemize}
    \item Intensive composition ($\tau$, machinery share) is positive and highly significant.
    \item The interaction is \textbf{strongly negative and significant} during the Fordist era.
  \end{itemize}
\end{frame}
  ...
  \begin{itemize}
    ...
    \item \textbf{Next Steps (Stage S40):}
      \begin{itemize}
        ...
        \item Proceed to the Stage B investment function estimation using these recovered objects.
      \end{itemize}
  \end{itemize}
\end{frame}
