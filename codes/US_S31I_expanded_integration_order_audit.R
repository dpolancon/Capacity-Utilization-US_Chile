...
# S31I audits integration order and I(2) risk. It estimates no regression,
...
output_dir <- file.path(repo_root, "data", "processed", "us_s31i")
...
output_paths <- c(
  panel = file.path(output_dir, "us_s31i_candidate_audit_panel.csv"),
  registry = file.path(output_dir, "us_s31i_variable_registry.csv"),
  tests = file.path(output_dir, "us_s31i_integration_order_tests.csv"),
  i2_ledger = file.path(output_dir, "us_s31i_i2_risk_ledger.csv"),
  rolling = file.path(output_dir, "us_s31i_rolling_window_audit.csv"),
  recommendations = file.path(
    output_dir, "us_s31i_admissibility_recommendations.csv"
  ),
  checks = file.path(output_dir, "us_s31i_validation_checks.csv"),
  validation_report = file.path(
    validation_dir, "US_S31I_EXPANDED_INTEGRATION_ORDER_AUDIT_VALIDATION.md"
  ),
  advisor_report = file.path(
    results_dir, "US_S31I_ADVISOR_INTEGRATION_AUDIT_TABLES.md"
  )
)
...
if (length(missing_inputs) > 0L) {
  abort(paste("Missing S31I inputs:", paste(missing_inputs, collapse = "\n")))
}
...
if (file.exists(s21_path)) {
  ...
  aggregate_q_status <- "constructed_for_S31I_integration_audit"
}
...
for (name in c(
  ...
  add_registry(
    ...
    "candidate_only", ifelse(startsWith(name, "Delta_"), "S31I", "S20"),
...
for (name in aggregate_q_names) {
  ...
  add_registry(
    ...
    ifelse(file.exists(s21_path), "S21", "S31I"),
...
for (name in names(mechanization_q_specs)) {
  add_registry(
    ...
    "mechanization_bias_candidate", "candidate_only", "S31I",
    paste0(
...
advisor_lines <- c(
  ...
  "",
  paste(
    "S31I audits integration-order admissibility and I(2) risk for the",
...
checks <- do.call(rbind, list(
  ...
  validation_check("aggregate_q_not_demoted",
    ...
    if (
      ...
      ] %in% c("canonical_S21_output", "constructed_for_S31I_integration_audit"))
    ) "PASS" else "FAIL",
  ...
  validation_check("audit_not_model_choice",
    ...
    "All outputs are S31I audit artifacts."),
  ...
  validation_check("baseline_i1_carry",
    ...
    paste(
      ...
      "candidates; no estimator is run in S31I."
    )),
  ...
  validation_check("s21_optional", "Canonical S21 q output availability",
    ...
    if (file.exists(s21_path)) "Canonical S21 panel audited."
      ...
      "Missing optional S21; aggregate q variants constructed for S31I",
...
validation_lines <- c(
  ...
  "",
  paste(
    "S31I audits integration order, I(2) risk, and historical-window",
  ...
  "",
  paste(
    "S31I fetched no BEA data, modified no provider or upstream output,",
...
if (any(checks$status == "FAIL")) {
  abort(paste(
    "S31I validation failed:",
    paste(checks$check_name[checks$status == "FAIL"], collapse = "; ")
  ))
}
message("S31I expanded integration-order audit passed.")
