...
stage_dir <- file.path(repo_root, "output", "S34R_B_cpr_realigned_design_gate")
csv_dir <- file.path(stage_dir, "csv")
input_path <- file.path(csv_dir, "S34R_B_repaired_augmented_panel.csv")
spec_path <- file.path(csv_dir, "S34R_B_eg_model_specs.csv")
output_path <- file.path(csv_dir, "S34R_B_residual_cointegration_eg_ledger.csv")

if (!requireNamespace("aTSA", quietly = TRUE)) {
  install.packages("aTSA", repos = "https://cloud.r-project.org", quiet = TRUE)
}
if (!requireNamespace("aTSA", quietly = TRUE)) {
  stop("aTSA package is required for S34R-B EG gate and could not be installed.", call. = FALSE)
}
