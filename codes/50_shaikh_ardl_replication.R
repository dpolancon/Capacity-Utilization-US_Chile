...
# ============================================================
# 50_shaikh_ardl_replication_WRAPPED.R  (PATCH CONSOLIDATE)
#
# Wrap-up runner:
...
# -----------------------------
# 4) Bounds tests (CASE aligned with model deterministics)
...
# -----------------------------
# 5) Long-run multipliers table (package) + dummy LR multipliers (Shaikh scaling)
# -----------------------------
...
lnYp_hat_dummy   <- a_lr + b_lr * df$lnK + dummy_effect
lnYp_hat_nodummy <- a_lr + b_lr * df$lnK

u_hat_shaikh_ardl_rep_dummy   <- exp(df$lnY - lnYp_hat_dummy)
u_hat_shaikh_ardl_rep_nodummy <- exp(df$lnY - lnYp_hat_nodummy)
...
cat("\n--- Long-run multipliers (including Shaikh-scaled dummies) ---\n")
print(lr_full_table)

cat("\n--- Utilization diagnostics ---\n")
cat("mean(u_dummy)   = ", round(mean(u_hat_shaikh_ardl_rep_dummy,   na.rm = TRUE), 4), "\n", sep = "")
cat("sd(u_dummy)     = ", round(sd(u_hat_shaikh_ardl_rep_dummy,     na.rm = TRUE), 4), "\n", sep = "")
cat("mean(u_nodummy) = ", round(mean(u_hat_shaikh_ardl_rep_nodummy, na.rm = TRUE), 4), "\n", sep = "")
cat("sd(u_nodummy)   = ", round(sd(u_hat_shaikh_ardl_rep_nodummy,   na.rm = TRUE), 4), "\n", sep = "")

if ("u_shaikh" %in% names(df)) {
  cat("\n--- Correlation with Shaikh series ---\n")
  cat("cor(u_dummy,  u_shaikh) = ", round(cor(u_hat_shaikh_ardl_rep_dummy,   df$u_shaikh, use = "complete.obs"), 4), "\n", sep = "")
  cat("cor(u_nodummy,u_shaikh) = ", round(cor(u_hat_shaikh_ardl_rep_nodummy, df$u_shaikh, use = "complete.obs"), 4), "\n", sep = "")
} else {
...
# Convert ARDL to unrestricted ECM form
...
uecm_coefs <- summary(uecm_model)$coefficients
...
# -----------------------------
# RECM (restricted ECM): λ is coef on ECT
# Try both intercept placements, then choose the one that matches Shaikh.
# -----------------------------

cat("\n--- RECM (candidate A): intercept unrestricted (short-run) ---\n")
recm_A <- ARDL::recm(fit_ardl, case = 3)   # per ARDL manual example
print(summary(recm_A))

cat("\n--- RECM (candidate B): intercept in long-run (inside ECT) ---\n")
recm_B <- ARDL::recm(fit_ardl, case = 2)   # commonly the 'restricted intercept' alternative
print(summary(recm_B))

# -----------------------------
# Extract and print λ from RECM (same presentation style)
# -----------------------------


print_lambda(uecm_model,
             ect_pattern = "^L\\(lnY, 1\\)$",
             model_label = "UECM")

print_lambda(recm_A,
             ect_pattern = "^ect$",
             model_label = "RECM Case 3")

print_lambda(recm_B,
             ect_pattern = "^ect$",
             model_label = "RECM Case 2")
...
# -----------------------------
# 9) Single minimalistic plot (3 series) + save PNG
# -----------------------------
df_plot <- data.frame(
  year      = df$year,
  u_dummy   = u_hat_shaikh_ardl_rep_dummy,
...
plot_u_hat_shaikh_ardl_rep <- ggplot(df_plot, aes(x = year)) +
  ...
  geom_line(aes(y = u_dummy,   color = "Replication (no LR dummies in Y^p)"),
            linetype = "dashed", size = 0.8) +
  geom_line(aes(y = u_nodummy, color = "Replication (LR dummies in Y^p)"), 
            size = 0.8) +
  geom_hline(yintercept = 1, linetype = "solid", alpha = 0.4) +
  geom_vline(xintercept = dummy_years, linetype = "dashed", alpha = 0.6) +
  scale_color_manual(values = c(
    "Shaikh (2016)"                      = "red",
    "Replication (LR dummies in Y^p)"    = "black",
    "Replication (no LR dummies in Y^p)" = "steelblue"
  )) +
...
ggsave(fig_path, plot_u_hat_shaikh_ardl_rep, width = 10, height = 4.5, dpi = 300)

cat("\nSaved figure:\n  ", fig_path, "\n", sep = "")
cat("Saved log:\n  ", log_path, "\n", sep = "")

# -----------------------------
# 10) Close sink (IMPORTANT)
# -----------------------------
sink()