# S24B constructs only the authorized fixed-assets source-input family
...
target_family <- "fixed_assets_source_inputs"
previous_family <- "income_distribution_source_inputs"
expected_family_count <- 56L
expected_panel_rows <- 5936L
final_pass_decision <- "AUTHORIZE_S24C_PROVIDER_SOURCE_INPUTS_OTHER_CONSTRUCTION"
final_pass_status <- "S24B_FIXED_ASSETS_SOURCE_INPUTS_CONSTRUCTION_COMPLETE_S24C_AUTHORIZED"
final_fail_decision <- "BLOCK_FOR_FIXED_ASSETS_CONSTRUCTION_REVIEW"
final_fail_status <- "S24B_FIXED_ASSETS_SOURCE_INPUTS_CONSTRUCTION_BLOCKED_FOR_REVIEW"
...
csv_dir <- file.path(s24b_dir, "csv")
md_dir <- file.path(s24b_dir, "md")
...
output_paths <- list(
  panel = path(csv_dir, "S24B_fixed_assets_source_inputs_long.csv"),
  ledger = path(csv_dir, "S24B_fixed_assets_construction_ledger.csv"),
  provenance = path(csv_dir, "S24B_fixed_assets_provenance_audit.csv"),
  exclusion = path(csv_dir, "S24B_fixed_assets_exclusion_audit.csv"),
  continuity = path(csv_dir, "S24B_fixed_assets_continuity_audit.csv"),
  validation = path(csv_dir, "S24B_validation_checks.csv"),
  validation_md = path(md_dir, "S24B_FIXED_ASSETS_SOURCE_INPUTS_VALIDATION.md"),
  decision_md = path(md_dir, "S24B_FIXED_ASSETS_SOURCE_INPUTS_DECISION.md")
)
...
s24a_decision_clean <- grepl(
  "AUTHORIZE_S24B_FIXED_ASSETS_SOURCE_INPUTS_CONSTRUCTION",
  s24a_decision_text,
  fixed = TRUE
)

if (!s23_validation_clean || !s23_decision_clean) {
  stop("S23 validation or decision is not clean; S24B must stop.", call. = FALSE)
}
if (!s24a_validation_clean || !s24a_decision_clean) {
  stop("S24A validation or decision is not clean; S24B must stop.", call. = FALSE)
}
...
fixed_queue <- s23_queue[s23_queue$construction_family == target_family, , drop = FALSE]
fixed_queue <- fixed_queue[order(to_int(fixed_queue$queue_rank), fixed_queue$variable_id), , drop = FALSE]
fixed_ids <- fixed_queue$variable_id

s23_meta <- data.frame(
  variable_id = fixed_queue$variable_id,
  s23_queue_rank = to_int(fixed_queue$queue_rank),
  display_name = fixed_queue$display_name,
  concept_family = fixed_queue$concept_family,
  construction_family = fixed_queue$construction_family,
  sector_boundary = fixed_queue$sector_boundary,
  account_position = fixed_queue$account_position,
  s23_candidate_status = fixed_queue$candidate_status,
  s23_object_admissibility = fixed_queue$object_admissibility,
  s23_source_observation_rows = to_int(fixed_queue$source_observation_rows),
  s23_coverage_start = to_int(fixed_queue$coverage_start),
  s23_coverage_end = to_int(fixed_queue$coverage_end),
  s23_authorization_scope = fixed_queue$s23_authorization_scope,
  s23_can_construct_in_s23 = fixed_queue$can_construct_in_s23,
  s23_can_model_in_s23 = fixed_queue$can_model_in_s23,
  s23_required_before_implementation = fixed_queue$required_before_implementation,
  s23_notes = fixed_queue$notes_s23,
  stringsAsFactors = FALSE
)

s22_fixed <- s22_authorized[s22_authorized$variable_id %in% fixed_ids, , drop = FALSE]
s22_meta <- data.frame(
  variable_id = s22_fixed$variable_id,
  s22_display_name = s22_fixed$display_name,
  s22_concept_family = s22_fixed$concept_family,
  s22_sector_boundary = s22_fixed$sector_boundary,
  s22_account_position = s22_fixed$account_position,
  s22_source_status = s22_fixed$source_status,
  s22_candidate_status = s22_fixed$candidate_status,
  s22_object_admissibility = s22_fixed$object_admissibility,
  s22_source_observation_rows = to_int(s22_fixed$source_observation_rows),
  s22_coverage_start = to_int(s22_fixed$coverage_start),
  s22_coverage_end = to_int(s22_fixed$coverage_end),
  s22_authorization = s22_fixed$s22_authorization,
  s22_can_construct_variable_now = s22_fixed$can_construct_variable_now,
  s22_can_model_now = s22_fixed$can_model_now,
  s22_can_run_econometrics_now = s22_fixed$can_run_econometrics_now,
  s22_notes = s22_fixed$s22_notes,
  stringsAsFactors = FALSE
)

fixed_panel <- s21_panel[s21_panel$variable_id %in% fixed_ids, , drop = FALSE]
fixed_panel$year <- to_int(fixed_panel$year)
fixed_panel$value <- as.numeric(fixed_panel$value)
fixed_panel <- merge(fixed_panel, s23_meta, by = "variable_id", all.x = TRUE, sort = FALSE)
fixed_panel <- merge(fixed_panel, s22_meta, by = "variable_id", all.x = TRUE, sort = FALSE)
fixed_panel$s24b_stage_id <- "S24B_FIXED_ASSETS_SOURCE_INPUTS_CONSTRUCTION"
fixed_panel$s24b_construction_action <- "source_input_materialized_from_s21_downstream_intake"
fixed_panel$s24b_authorization_scope <- "fixed_assets_source_inputs_only"
fixed_panel$provider_v1_commit <- provider_v1_commit
fixed_panel$s21_intake_commit <- s21_commit
fixed_panel$s22_model_input_preparation_commit <- s22_commit
fixed_panel$s23_construction_plan_commit <- s23_commit
fixed_panel$s24a_income_distribution_construction_commit <- s24a_commit
fixed_panel$s24b_constructed_object <- "yes"
fixed_panel$s24b_modeling_authorized <- "no"
fixed_panel$s24b_econometrics_authorized <- "no"
fixed_panel$s24b_notes <- "S24B materializes fixed-assets source-input rows only; no GPIM, theta, productive capacity, utilization, accumulated q, adjusted Shaikh, model, or econometric object is constructed."
fixed_panel <- fixed_panel[order(fixed_panel$s23_queue_rank, fixed_panel$year), , drop = FALSE]

ledger_rows <- lapply(fixed_ids, function(id) {
  rows <- fixed_panel[fixed_panel$variable_id == id, , drop = FALSE]
  ...
  data.frame(
    ...
    s24b_construction_status = if (nrow(rows) == meta$s23_source_observation_rows) "constructed" else "row_count_mismatch",
    ...
    s24b_decision_effect = "constructed_as_authorized_fixed_assets_source_input_only",
...
provenance_rows <- lapply(fixed_ids, function(id) {
  rows <- fixed_panel[fixed_panel$variable_id == id, , drop = FALSE]
  data.frame(
    variable_id = id,
...
sequence_counts <- data.frame(
  ...
  s24b_constructed_object_count = vapply(
  ...
  s24b_action = ifelse(
    ...
    "constructed_in_s24b",
    "excluded_from_s24b_by_family_boundary"
  ),
  stringsAsFactors = FALSE
)
...
deferred_summary <- data.frame(
  ...
  s24b_constructed_object_count = 0L,
  s24b_action = "excluded_from_s24b_deferred_or_blocked_boundary",
  stringsAsFactors = FALSE
)
...
continuity_audit <- data.frame(
  audit_item = c(
    ...
    "s24a_decision_authorizes_s24b",
    "s24b_only_adds_fixed_assets_family"
  ),
  status = c(
    ...
    if (s24a_decision_clean) "PASS" else "FAIL",
    if (all(ledger$construction_family == target_family) && !any(s24a_ledger$construction_family == target_family)) "PASS" else "FAIL"
  ),
  evidence = c(
    ...
    "AUTHORIZE_S24B_FIXED_ASSETS_SOURCE_INPUTS_CONSTRUCTION",
    paste0(nrow(ledger), " S24B fixed-assets objects; 0 S24A fixed-assets objects")
  ),
  stringsAsFactors = FALSE
)
...
lineage_ok <- all(fixed_panel$provider_release_commit == provider_v1_commit) &&
  all(fixed_panel$provider_v1_commit == provider_v1_commit) &&
  all(fixed_panel$s21_intake_commit == s21_commit) &&
  all(fixed_panel$s22_model_input_preparation_commit == s22_commit) &&
  all(fixed_panel$s23_construction_plan_commit == s23_commit) &&
  all(fixed_panel$s24a_income_distribution_construction_commit == s24a_commit)
...
add_check("s24a_decision_authorizes_s24b", s24a_decision_clean, "AUTHORIZE_S24B_FIXED_ASSETS_SOURCE_INPUTS_CONSTRUCTION")
...
add_check("fixed_assets_family_filtered", all(fixed_queue$construction_family == target_family), target_family)
add_check("fixed_assets_family_count_equals_56", nrow(fixed_queue) == expected_family_count, paste0(nrow(fixed_queue), " objects"))
add_check("no_income_distribution_objects_reconstructed", !any(ledger$construction_family == previous_family), "0 income-distribution constructed objects in S24B")
...
add_check("no_adjusted_shaikh_objects_constructed", all(fixed_panel$can_construct_adjusted_shaikh == "no"), "all constructed rows retain can_construct_adjusted_shaikh=no")
...
add_check("constructed_fixed_assets_panel_created", nrow(fixed_panel) == expected_panel_rows && row_count_ok && coverage_ok, paste0(nrow(fixed_panel), " panel rows"))
...
add_check("exclusion_audit_created", nrow(exclusion_audit) == 7L && all(exclusion_audit$s24b_constructed_object_count[exclusion_audit$construction_family != target_family] == 0L), paste0(nrow(exclusion_audit), " exclusion rows"))
add_check("continuity_audit_created", nrow(continuity_audit) == 6L && all(continuity_audit$status == "PASS"), paste0(nrow(continuity_audit), " continuity rows"))
add_check("provider_v1_commit_preserved", all(fixed_panel$provider_release_commit == provider_v1_commit), provider_v1_commit)
add_check("s21_lineage_preserved", lineage_ok && all(fixed_panel$s21_intake_commit == s21_commit), s21_commit)
add_check("s22_lineage_preserved", lineage_ok && all(fixed_panel$s22_model_input_preparation_commit == s22_commit), s22_commit)
add_check("s23_lineage_preserved", lineage_ok && all(fixed_panel$s23_construction_plan_commit == s23_commit), s23_commit)
add_check("s24a_lineage_preserved", lineage_ok && all(fixed_panel$s24a_income_distribution_construction_commit == s24a_commit), s24a_commit)
add_check("no_modeling_outputs_created", all(fixed_panel$s24b_modeling_authorized == "no"), "S24B emits only source-input panel and audits")
add_check("no_econometric_outputs_created", all(fixed_panel$s24b_econometrics_authorized == "no"), "No econometric output paths emitted")
...
add_check("no_provider_repo_modification", TRUE, "Provider repo not written; S24B consumes downstream S21/S22/S23/S24A files only")

validation <- add_check(flush = TRUE)
clean <- nrow(validation) == 35L && all(validation$status == "PASS")
...
write_csv(fixed_panel, output_paths$panel)
...
validation_md <- c(
  "# S24B Fixed Assets Source Inputs Validation",
  "",
  paste0("S24B validation result: `", if (clean) "PASS" else "FAIL", " ", sum(validation$status == "PASS"), "`"),
  "",
  paste0("Constructed family: `", target_family, "`."),
  paste0("Constructed object count: `", nrow(ledger), "`."),
  paste0("Constructed panel row count: `", nrow(fixed_panel), "`."),
  "",
  "S24B constructs only fixed-assets source-input observations from the downstream S21 intake, filtered through the S23 authorized build queue. It reconstructs no income-distribution objects and constructs no provider-other objects, adjusted Shaikh objects, modeling panels, econometric outputs, GPIM, theta, productive capacity, utilization, or accumulated q.",
  "",
  "## Validation checks",
  "",
  ...
  "",
  md_table(exclusion_audit, c("construction_family", "authorized_object_count", "s24b_constructed_object_count", "s24b_action")),
  "",
  "## Continuity audit",
  "",
...
decision_md <- c(
  "# S24B Fixed Assets Source Inputs Decision",
  "",
  paste0("Decision: `", final_decision, "`"),
  "",
  paste0("Final status: `", final_status, "`"),
  "",
  paste0("S24B consumed S24A commit `", s24a_commit, "`, S23 commit `", s23_commit, "`, S22 commit `", s22_commit, "`, S21 commit `", s21_commit, "`, and provider V1 commit `", provider_v1_commit, "`."),
  "",
  paste0("Constructed family: `", target_family, "`."),
  paste0("Constructed object count: `", nrow(ledger), "`."),
  paste0("Constructed panel row count: `", nrow(fixed_panel), "`."),
  "",
  "This decision authorizes only the next bounded construction family, S24C provider-source-inputs-other construction. It does not authorize modeling, econometrics, GPIM, theta, productive capacity, utilization, accumulated q, adjusted Shaikh reconstruction, income-distribution reconstruction, or promotion of deferred, blocked, parked, documentation-only, or theoretically unresolved objects.",
  "",
  "S24B stops here."
)
...
if (!clean) {
  ...
  stop("S24B validation failed:\n- ", paste(failed, collapse = "\n- "), call. = FALSE)
}

message("S24B validation PASS: ", sum(validation$status == "PASS"))
message("Constructed panel rows: ", nrow(fixed_panel))
