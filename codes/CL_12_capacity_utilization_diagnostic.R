...
quantity_candidates <- list(
  X_q = c("X_q", "x_q", "X_real", "x_real", "exports_real", "exports_volume", "x_vol", "Xvol"),
  M_q = c("M_q", "m_q", "M_real", "m_real", "imports_real", "imports_volume", "m_vol", "Mvol"),
  X_nom = c("X", "x", "X_nom", "exports_nominal", "exports_current", "exports_value"),
  M_nom = c("M", "m", "M_nom", "imports_nominal", "imports_current", "imports_value")
)
...
col_X_q  <- resolve_optional(profit_df, MANUAL_MAP$X_q, quantity_candidates$X_q)
col_M_q  <- resolve_optional(profit_df, MANUAL_MAP$M_q, quantity_candidates$M_q)
col_X_nom <- resolve_optional(profit_df, MANUAL_MAP$X_nom, quantity_candidates$X_nom)
col_M_nom <- resolve_optional(profit_df, MANUAL_MAP$M_nom, quantity_candidates$M_nom)

if (is.na(col_X_q) && is.na(col_X_nom)) {
  stop("Could not resolve exports quantity/value input. Set MANUAL_MAP$X_q or MANUAL_MAP$X_nom.", call. = FALSE)
}
if (is.na(col_M_q) && is.na(col_M_nom)) {
  stop("Could not resolve imports quantity/value input. Set MANUAL_MAP$M_q or MANUAL_MAP$M_nom.", call. = FALSE)
}
