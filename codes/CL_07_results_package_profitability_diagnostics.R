...
#
# Purpose:
# - Identify Chile profit swings using a joint diagnostic:
...
# - Produce transparent CSV outputs for discussion
# - Produce a U.S.-style diagnostic figure:
#     solid line  = profit rate in level
#     dashed line = smoothed Δ log(profitability), secondary axis
# - Produce a second figure:
#     standardized smoothed Δ log(profitability) vs standardized ECT
#
# Detection ladder:
# - primary: sign reversals in smoothed Δlog(r)
# - fallback 1: sign reversals in raw Δlog(r)
# - fallback 2: local extrema in profit-rate levels
#
# Key rules:
# - dating is assigned to local extrema of the profit-rate level
# - RHS terminal window is NOT dropped
# - if no internal turning points exist, the script still exports diagnostics
###############################################################################
...
out_root <- file.path(proj_root, "output", "profitability_chile_diagnostic")
...
# Carrier settings
level_var <- "r_diag_t"      # can switch to "r_struct_t" if needed
log_var   <- "ln_r_diag_t"   # can switch to "ln_r_struct_t" if needed
ect_var   <- "ect"
...
prune_turning_points_joint <- function(tp, signal_tbl, min_years = 2L, min_log_move = 0.03) {
  if (nrow(tp) <= 1) return(tp)
...
# ── 6. Save outputs ──────────────────────────────────────────────────────────
readr::write_csv(config_tbl, file.path(csv_dir, "diagnostic_config_chile_profit_swings.csv"))
readr::write_csv(tp_status, file.path(csv_dir, "turning_points_status_chile_profit_swings.csv"))

readr::write_csv(signal_tbl, file.path(csv_dir, "signal_table_chile_profit_swings.csv"))
readr::write_csv(tp_candidates_primary, file.path(csv_dir, "turning_points_candidates_primary_chile_profit_swings.csv"))
readr::write_csv(tp_candidates_raw, file.path(csv_dir, "turning_points_candidates_raw_chile_profit_swings.csv"))
readr::write_csv(tp_candidates_level, file.path(csv_dir, "turning_points_candidates_level_chile_profit_swings.csv"))
readr::write_csv(tp_candidates_used, file.path(csv_dir, "turning_points_candidates_used_chile_profit_swings.csv"))
readr::write_csv(tp_final_pruned, file.path(csv_dir, "turning_points_final_pruned_chile_profit_swings.csv"))
readr::write_csv(tp_used, file.path(csv_dir, "turning_points_used_chile_profit_swings.csv"))
readr::write_csv(tp_bounds, file.path(csv_dir, "turning_points_with_boundaries_chile_profit_swings.csv"))
readr::write_csv(swings_tbl, file.path(csv_dir, "profit_swings_joint_diagnostic_chile.csv"))

saveRDS(config_tbl, file.path(rds_dir, "diagnostic_config_chile_profit_swings.rds"))
saveRDS(tp_status, file.path(rds_dir, "turning_points_status_chile_profit_swings.rds"))
saveRDS(signal_tbl, file.path(rds_dir, "signal_table_chile_profit_swings.rds"))
saveRDS(tp_candidates_primary, file.path(rds_dir, "turning_points_candidates_primary_chile_profit_swings.rds"))
saveRDS(tp_candidates_raw, file.path(rds_dir, "turning_points_candidates_raw_chile_profit_swings.rds"))
saveRDS(tp_candidates_level, file.path(rds_dir, "turning_points_candidates_level_chile_profit_swings.rds"))
saveRDS(tp_candidates_used, file.path(rds_dir, "turning_points_candidates_used_chile_profit_swings.rds"))
saveRDS(tp_final_pruned, file.path(rds_dir, "turning_points_final_pruned_chile_profit_swings.rds"))
saveRDS(tp_used, file.path(rds_dir, "turning_points_used_chile_profit_swings.rds"))
saveRDS(tp_bounds, file.path(rds_dir, "turning_points_with_boundaries_chile_profit_swings.rds"))
saveRDS(swings_tbl, file.path(rds_dir, "profit_swings_joint_diagnostic_chile.rds"))

# ── 7. Figures ───────────────────────────────────────────────────────────────
plot_joint_diagnostic(
  signal_tbl = signal_tbl,
  tp_used = tp_used,
  out_png = file.path(fig_dir, "fig01_chile_profit_turning_points_joint.png"),
  out_pdf = file.path(fig_dir, "fig01_chile_profit_turning_points_joint.pdf"),
  title_txt = NULL
)
...
plot_ect_diagnostic(
  signal_tbl = signal_tbl,
  tp_used = tp_used,
  out_png = file.path(fig_dir, "fig02_chile_profit_vs_ect_diagnostic.png"),
  out_pdf = file.path(fig_dir, "fig02_chile_profit_vs_ect_diagnostic.pdf"),
  title_txt = NULL
)

# ── 8. Console summary ───────────────────────────────────────────────────────
cat("Saved Chile profit swings diagnostic outputs to:\n")
cat("  ", out_root, "\n", sep = "")
cat("Files:\n")
cat("  csv/diagnostic_config_chile_profit_swings.csv\n")
cat("  csv/turning_points_status_chile_profit_swings.csv\n")
cat("  csv/signal_table_chile_profit_swings.csv\n")
cat("  csv/turning_points_candidates_primary_chile_profit_swings.csv\n")
cat("  csv/turning_points_candidates_raw_chile_profit_swings.csv\n")
cat("  csv/turning_points_candidates_level_chile_profit_swings.csv\n")
cat("  csv/turning_points_candidates_used_chile_profit_swings.csv\n")
cat("  csv/turning_points_final_pruned_chile_profit_swings.csv\n")
cat("  csv/turning_points_used_chile_profit_swings.csv\n")
cat("  csv/turning_points_with_boundaries_chile_profit_swings.csv\n")
cat("  csv/profit_swings_joint_diagnostic_chile.csv\n")
cat("  figures/fig01_chile_profit_turning_points_joint.png\n")
cat("  figures/fig02_chile_profit_vs_ect_diagnostic.png\n")
cat("  rds/*.rds\n")