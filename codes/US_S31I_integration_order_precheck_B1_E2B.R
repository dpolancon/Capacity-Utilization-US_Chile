###############################################################################
# US_S31I_integration_order_precheck_B1_E2B.R
# Chapter 2 - US S31I integration-order precheck for S32 B1/E2B
#
# Role:
...
RUN_TAG <- "S31I_INTEGRATION_ORDER_PRECHECK_B1_E2B"
...
LOG_PATH <- file.path(logs_dir, "S31I_execution_log.txt")
...
write_csv_base(input_audit, file.path(audit_dir, "S31I_input_columns_audit.csv"))
...
write_csv_base(ledger, file.path(csv_dir, "S31I_variable_construction_ledger.csv"))
...
write_csv_base(tests_long, file.path(csv_dir, "S31I_integration_order_tests_long.csv"))
write_csv_base(classification, file.path(csv_dir, "S31I_integration_order_classification.csv"))
...
writeLines(report_lines, file.path(md_dir, "S31I_INTEGRATION_ORDER_PRECHECK_B1_E2B.md"), useBytes = TRUE)
writeLines(c("# S31I Validation Report", "", "Validation pending."), file.path(md_dir, "S31I_VALIDATION_REPORT.md"), useBytes = TRUE)
...
required_csv <- file.path(csv_dir, c(
  "S31I_integration_order_tests_long.csv",
  "S31I_integration_order_classification.csv",
  "S31I_variable_construction_ledger.csv"
))
required_md <- file.path(md_dir, c(
  "S31I_INTEGRATION_ORDER_PRECHECK_B1_E2B.md",
  "S31I_VALIDATION_REPORT.md"
))
...
write_csv_base(validation, file.path(audit_dir, "S31I_validation_checks.csv"))

validation_lines <- c(
  "# S31I Validation Report",
  "",
...
writeLines(validation_lines, file.path(md_dir, "S31I_VALIDATION_REPORT.md"), useBytes = TRUE)

if (!all(validation$pass)) {
  log_msg("Validation failed. See ", file.path(md_dir, "S31I_VALIDATION_REPORT.md"))
  stop("S31I validation failed.", call. = FALSE)
}

log_msg("S31I completed successfully. Output folder: ", out_dir)
cat("\nFinal S31I summary\n")
cat("Run tag: ", RUN_TAG, "\n", sep = "")
...
cat("S31I integration-order precheck implemented. S31 VIF screen left untouched. S32 model-choice outputs left untouched. S40 remains parked.\n")
