...
# 3) build reduced-form offensive / defensive decomposition
# 4) identify endogenous class-struggle swings from wage-share peak-to-trough
# 5) decompose each swing
# 6) compare class-struggle dynamics against fixed profit-rate swing shell
#
# Source:
# - Astorga2023_SupM3.xlsx, sheet "Chile", range A1:Q99
#
# Core reduced-form identity:
...
#
# Output root:
# - output/chile_class_struggle_astorga_weisskopf/
###############################################################################
...
# Merge only short / shallow interruptions inside larger trends
max_interrupt_years <- 3L
max_interrupt_log_drop <- 0.10
...
find_tp_from_signal <- function(df, signal_col, level_col, span = 1L,
                                eps = 1e-10, method_name = "signal_rule") {
  year <- df$year
  level <- df[[level_col]]
  sig <- df[[signal_col]]
...
find_tp_from_levels <- function(df, level_col, span = 1L) {
  year <- df$year
  level <- df[[level_col]]
...
collapse_nested_progressions <- function(tp,
                                         max_interrupt_years = 3L,
                                         max_interrupt_log_drop = 0.10) {
  if (nrow(tp) < 4) return(tp)
  ...
  repeat {
    ...
    for (i in seq_len(nrow(out) - 3)) {
      types <- out$type[i:(i + 3)]
      yrs   <- out$year[i:(i + 3)]
      vals  <- out$value[i:(i + 3)]

      # trough -> peak -> trough -> peak
      if (identical(types, c("trough", "peak", "trough", "peak"))) {
        interrupt_years <- yrs[3] - yrs[2]
        interrupt_drop  <- abs(log(vals[3] / vals[2]))

        if (vals[3] > vals[1] &&
            vals[4] > vals[2] &&
            interrupt_years <= max_interrupt_years &&
            interrupt_drop  <= max_interrupt_log_drop) {
          out <- out[-c(i + 1, i + 2), , drop = FALSE]
          changed <- TRUE
          break
        }
      }
      ...
      if (identical(types, c("peak", "trough", "peak", "trough"))) {
        interrupt_years <- yrs[3] - yrs[2]
        interrupt_rise  <- abs(log(vals[3] / vals[2]))

        if (vals[3] < vals[1] &&
            vals[4] < vals[2] &&
            interrupt_years <= max_interrupt_years &&
            interrupt_rise  <= max_interrupt_log_drop) {
          out <- out[-c(i + 1, i + 2), , drop = FALSE]
          changed <- TRUE
          break
        }
...
# ── 5. Peak-to-trough analysis on wage-share series ──────────────────────────
signal_tbl <- astorga_decomp |>
  dplyr::mutate(
    dln_w_share_smoothed = zoo::rollmean(dln_w_share, k = k_smooth, fill = NA, align = "center"),
    w_share_smoothed = zoo::rollmean(w_share, k = k_smooth, fill = NA, align = "center"),
    dln_w_share_z = zsafe(dln_w_share),
    dln_w_share_smoothed_z = zsafe(dln_w_share_smoothed)
  )

tp_candidates_primary <- find_tp_from_signal(
  df = signal_tbl,
  signal_col = "dln_w_share_smoothed",
  level_col = "w_share",
  span = local_span,
  method_name = "smoothed_dln_rule"
) |>
  compress_turning_points()

tp_candidates_raw <- find_tp_from_signal(
  df = signal_tbl,
  signal_col = "dln_w_share",
  level_col = "w_share",
  span = local_span,
  method_name = "raw_dln_rule"
) |>
  compress_turning_points()
...
tp_major <- if (nrow(tp_final_pruned) > 0) {
  collapse_nested_progressions(
    tp_final_pruned,
    max_interrupt_years = max_interrupt_years,
    max_interrupt_log_drop = max_interrupt_log_drop
  )
} else {
  tp_final_pruned
}
...
if (nrow(tp_major) == 0) {
  ...
  tp_used <- tp_major
  tp_bounds <- add_boundaries_keep_rhs(tp_used, signal_tbl, "w_share")
}
...
tp_status <- tibble::tibble(
  ...
  n_after_nested_merge = nrow(tp_major),
  max_interrupt_years = max_interrupt_years,
  max_interrupt_log_drop = max_interrupt_log_drop,
...
fig01 <- ggplot(signal_tbl, aes(x = year)) +
  ...
  geom_line(
    aes(y = w_share_smoothed, linetype = "Centered 3-year smooth"),
    linewidth = 1.0
  ) +
  geom_point(
    data = tp_used,
...
readr::write_csv(tp_candidates_primary, file.path(csv_dir, "class_struggle_tp_candidates_primary.csv"))
readr::write_csv(tp_candidates_raw, file.path(csv_dir, "class_struggle_tp_candidates_raw.csv"))
readr::write_csv(tp_candidates_level, file.path(csv_dir, "class_struggle_tp_candidates_level.csv"))
