...
#   Spec 3ch: g_K ~ r + mu + pi
#   Spec 4ch: g_K ~ mu + B_real + PyPK + pi
...
run_ardl <- function(ds, spec_name, formula_obj, sample_start, sample_end,
  ...
  cat("\nTop 5:\n")
  print(top5)
  write_csv(top5, file.path(csv_dir, sprintf("stageC_US_%s_top5_models.csv", tag)))
  ...
  write_csv(coef_df, file.path(csv_dir, sprintf("stageC_US_%s_ardl_coefficients.csv", tag)))
  ...
  write_csv(bounds_res, file.path(csv_dir, sprintf("stageC_US_%s_bounds_tests.csv", tag)))
  ...
  write_csv(as.data.frame(lr), file.path(csv_dir,
            sprintf("stageC_US_%s_longrun_multipliers.csv", tag)))
  ...
  write_csv(ecm_df, file.path(csv_dir, sprintf("stageC_US_%s_ecm_coefficients.csv", tag)))
  ...
  write_csv(diag_df, file.path(csv_dir, sprintf("stageC_US_%s_diagnostics.csv", tag)))
...
compare_wwii <- function(ds, spec_name, formula_obj, sample_start, sample_end,
  ...
  write_csv(comp, file.path(csv_dir, sprintf("stageC_US_%s_wwii_comparison.csv", tag)))
  comp
}
...
# 1. 3ch, 1930-1973
results[["3ch_1930"]] <- run_ardl(ds, "3ch", g_K ~ r + mu + pi,
  1930, 1973, max_ord = 3, k = 3, csv_dir)

# 2. 4ch, 1930-1973
results[["4ch_1930"]] <- run_ardl(ds, "4ch", g_K ~ mu + B_real + PyPK + pi,
  1930, 1973, max_ord = 3, k = 4, csv_dir, run_wald = TRUE)

# 3. 3ch, 1940-1973
results[["3ch_1940"]] <- run_ardl(ds, "3ch", g_K ~ r + mu + pi,
  1940, 1973, max_ord = 2, k = 3, csv_dir)

# 4. 4ch, 1940-1973
results[["4ch_1940"]] <- run_ardl(ds, "4ch", g_K ~ mu + B_real + PyPK + pi,
  1940, 1973, max_ord = 2, k = 4, csv_dir, run_wald = TRUE)

# 5. 3ch, 1947-1974 (Fordist core + 1 year post)
results[["3ch_1947"]] <- run_ardl(ds, "3ch", g_K ~ r + mu + pi,
  1947, 1974, max_ord = 2, k = 3, csv_dir)

# 6. 4ch, 1947-1974
results[["4ch_1947"]] <- run_ardl(ds, "4ch", g_K ~ mu + B_real + PyPK + pi,
...
wwii_3ch <- compare_wwii(ds, "3ch", g_K ~ r + mu + pi,
  1930, 1973, max_ord = 3, k = 3, csv_dir)

wwii_4ch <- compare_wwii(ds, "4ch", g_K ~ mu + B_real + PyPK + pi,
...
print(summary_tbl)
write_csv(summary_tbl, file.path(csv_dir, "stageC_US_extended_summary.csv"))
...
sprintf("**Date:** %s | **Script:** 22_ardl_extended_samples_us.R", Sys.Date()),
"",
"---",
"",
"## 1. Overview",
