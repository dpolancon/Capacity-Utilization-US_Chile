###############################################################################
# US_S30I_i2_risk_and_rolling_audit_B1_E2B.R
# Chapter 2 - US S30I I2-risk and rolling integration-order audit
#
# Role:
#   Audit existing S30I I2-risk classifications and add bounded rolling
#   integration-order window-stability diagnostics for human adjudication.
#
# Guardrails:
#   - Reads existing S30I outputs; does not overwrite original S30I CSV/MD files.
...
RUN_TAG <- "S30I_I2_RISK_AND_ROLLING_AUDIT_B1_E2B"
S30I_TAG <- "S30I_INTEGRATION_ORDER_PRECHECK_B1_E2B"

panel_path <- file.path(REPO, "data/processed/US/us_s20_admissibility_panel.csv")
s30i_dir <- file.path(REPO, "output/US", S30I_TAG)
csv_dir <- file.path(s30i_dir, "csv")
md_dir <- file.path(s30i_dir, "md")
logs_dir <- file.path(s30i_dir, "logs")
audit_dir <- file.path(s30i_dir, "audit")

tests_path <- file.path(csv_dir, "S30I_integration_order_tests_long.csv")
classification_path <- file.path(csv_dir, "S30I_integration_order_classification.csv")
ledger_path <- file.path(csv_dir, "S30I_variable_construction_ledger.csv")
report_path <- file.path(md_dir, "S30I_INTEGRATION_ORDER_PRECHECK_B1_E2B.md")
validation_path <- file.path(md_dir, "S30I_VALIDATION_REPORT.md")
...
i2_log_path <- file.path(logs_dir, "S30I_i2_risk_audit_log.txt")
rolling_log_path <- file.path(logs_dir, "S30I_rolling_integration_order_audit_log.txt")
...
log_i2("Read existing S30I outputs.")
...
write_csv_base(trace, file.path(csv_dir, "S30I_i2_risk_test_trace.csv"))
...
write_csv_base(i2_audit, file.path(csv_dir, "S30I_i2_risk_audit_capital_variables.csv"))
...
rule_diagnostics <- data.frame(
  ...
  diagnostic_question = c(
    "Were existing S30I outputs readable?",
  ...
  result = c(
    ...
    "codes/US_S30I_integration_order_precheck_B1_E2B.R:435-480"
  ),
  evidence = c(
  ...
  recommended_action = c(
    ...
    ifelse(nrow(likely_artifact) > 0L, "Write corrected classification output with suffix _corrected.", "Do not patch original S30I outputs."),
    "Document the rule and keep failed/skipped rows excluded from audit read."
  ),
  stringsAsFactors = FALSE
)
write_csv_base(rule_diagnostics, file.path(csv_dir, "S30I_i2_risk_rule_diagnostics.csv"))
...
if (nrow(likely_artifact) > 0L) {
  ...
  write_csv_base(corrected_out, file.path(csv_dir, "S30I_integration_order_classification_corrected.csv"))
}
...
i2_report <- c(
  ...
  if (nrow(likely_artifact) > 0L) {
    c(
      ...
      "Exact code location: `codes/US_S30I_integration_order_precheck_B1_E2B.R:435-480`.",
    ...
    c(
      ...
      "Exact code location inspected: `codes/US_S30I_integration_order_precheck_B1_E2B.R:435-480`.",
  ...
  "The original S30I pass produced I2-risk, deterministic-sensitive, and fragile classifications. This audit separates genuine first-difference nonstationarity from deterministic sensitivity, short-window weakness, failed-test handling, and mixed ADF/PP/KPSS evidence.",
  "",
  "## 3. Target variables and original S30I windows",
...
writeLines(i2_report, file.path(md_dir, "S30I_I2_RISK_AUDIT.md"), useBytes = TRUE)

i2_validation <- data.frame(
  run_tag = RUN_TAG,
  check = c(
    "Existing S30I outputs found",
    "Classification CSV read",
    "Tests-long CSV read",
    "Construction ledger read",
    "All target variables audited",
    "All original S30I windows attempted",
  ...
  pass = c(
    ...
    file.exists(file.path(csv_dir, "S30I_i2_risk_rule_diagnostics.csv"))
  ),
  evidence = c(
    s30i_dir,
    classification_path,
    ...
    paste(targets_present, collapse = " | "),
    paste(unique(classification$window_id), collapse = " | "),
    "See S30I_i2_risk_rule_diagnostics.csv",
    file.path(csv_dir, "S30I_i2_risk_rule_diagnostics.csv")
  ),
  stringsAsFactors = FALSE
)
writeLines(c(
  "# S30I I2-Risk Audit Validation",
  "",
...
), file.path(md_dir, "S30I_I2_RISK_AUDIT_VALIDATION.md"), useBytes = TRUE)
...
write_csv_base(rolling_windows, file.path(csv_dir, "S30I_rolling_integration_order_windows.csv"))
...
write_csv_base(rolling_stability, file.path(csv_dir, "S30I_rolling_integration_order_stability.csv"))
...
writeLines(rolling_report, file.path(md_dir, "S30I_ROLLING_INTEGRATION_ORDER_AUDIT.md"), useBytes = TRUE)
...
required_new <- c(
  file.path(csv_dir, "S30I_i2_risk_audit_capital_variables.csv"),
  file.path(csv_dir, "S30I_i2_risk_test_trace.csv"),
  file.path(csv_dir, "S30I_i2_risk_rule_diagnostics.csv"),
  file.path(md_dir, "S30I_I2_RISK_AUDIT.md"),
  file.path(md_dir, "S30I_I2_RISK_AUDIT_VALIDATION.md"),
  i2_log_path,
  file.path(csv_dir, "S30I_rolling_integration_order_windows.csv"),
  file.path(csv_dir, "S30I_rolling_integration_order_stability.csv"),
  file.path(md_dir, "S30I_ROLLING_INTEGRATION_ORDER_AUDIT.md"),
  rolling_log_path
)
...
combined_validation <- data.frame(
  run_tag = RUN_TAG,
  check = c(
    "Existing S30I outputs were found",
    ...
    "All original S30I windows were attempted",
    ...
    "Original S30I outputs were not overwritten",
  ...
  pass = c(
    ...
    nrow(rule_diagnostics) > 0L,
    file.exists(file.path(csv_dir, "S30I_i2_risk_rule_diagnostics.csv")),
    all(file.exists(c(file.path(csv_dir, "S30I_rolling_integration_order_windows.csv"), file.path(csv_dir, "S30I_rolling_integration_order_stability.csv")))),
  ...
  evidence = c(
    ...
    file.path(csv_dir, "S30I_i2_risk_rule_diagnostics.csv"),
    file.path(csv_dir, "S30I_i2_risk_rule_diagnostics.csv"),
    paste(file.path(csv_dir, c("S30I_rolling_integration_order_windows.csv", "S30I_rolling_integration_order_stability.csv")), collapse = " | "),
...
writeLines(c(
  "# S30I I2-Risk and Rolling Audit Validation",
  "",
...
), file.path(md_dir, "S30I_I2_RISK_AND_ROLLING_AUDIT_VALIDATION.md"), useBytes = TRUE)
...
cat("\nFinal S30I I2-risk and rolling audit summary\n")
cat("Run tag: ", RUN_TAG, "\n", sep = "")
...
cat("S30I I2-risk and rolling-window audit implemented. Original S30I outputs preserved. S31 VIF screen left untouched. S32 model-choice outputs left untouched. S40 remains parked.\n")
