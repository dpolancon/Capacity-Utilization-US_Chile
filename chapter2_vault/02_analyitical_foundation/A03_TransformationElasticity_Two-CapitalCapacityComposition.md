...
## Core Claim
A03 decomposes the A00 aggregate transformation elasticity. Its governing identification rule is [[R_distribution_conditioned_theta_identification]], and its aggregate benchmark is [[A00_Aggregate_Transformation_Benchmark]].
...
In A00, $\theta_t$ is generated by the accumulated distribution-conditioned capital-growth index. In A03, $\theta^N(\pi,s)$ denotes the analytical normal-capacity decomposition behind that aggregate path once $K_t$ is opened into $K^{ME}$, $K^{NRC}$, and $s_t$.
...
The bridge between the notes is exact:
...
is A03's growth-rate logic, while:
...
is A00's aggregate reduced-form device for carrying that logic into a level-compatible cointegrating regression. The accumulated index does not substitute for A03's two-capital decomposition. It preserves the aggregate benchmark until the machinery and nonresidential-structures channels are opened explicitly.

---

## Locked Formulation
A03 defines transformation elasticity as a growth-rate mapping from capital accumulation into productive-capacity growth. A00's accumulated distribution-conditioned capital-growth index is its aggregate level-compatible econometric counterpart, not a substitute for the two-capital decomposition. A03 opens aggregate $\theta_t$ into the baseline capacity payoff of nonresidential structures ($\theta^{NRC}$) and the distributively conditioned payoff of machinery ($\theta^{ME}$), mapping accumulation through $K^{ME}$, $K^{NRC}$, and $s_t$. Induced innovation operates through $\theta^{ME}$ by shifting the optimal mechanization rate $q^*(\pi)$ along a regime-conditioned frontier.
